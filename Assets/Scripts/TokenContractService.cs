...
public class TokenContractService : MonoBehaviour
{
    ...
    public string mainNetUrl, testNetUrl, mainNetContract, testNetContract;
    ...
    void Awake()
    {
        ...
        _url = GetComponent<WalletManager>().networkUrl;
        ...
        GetComponent<WalletManager>().AddInfoText("[Loading Token Info...]", true);
        GetComponent<WalletManager>().LoadingIndicator.SetActive(true);
    ...
    public void ChangeNet()
    {
        switch(mainnet)
        {
            case true:
                mainnet = false;
                TokenContractAddress = testNetContract;
                GetComponent<WalletManager>().networkUrl = testNetUrl;
                GetComponent<WalletManager>().RefreshBalance();
                break;
            case false:
                mainnet = true;
                TokenContractAddress = mainNetContract;
                GetComponent<WalletManager>().networkUrl = mainNetContract;
                GetComponent<WalletManager>().RefreshBalance();
                break;
        }
    }
    ...
    {
        WalletData wd = GetComponent<WalletManager>().GetSelectedWalletData();

        if (wd != null)
        {
            StartCoroutine(SendFunds(wd.address, GetComponent<WalletManager>().recepientAddressDropdown.options[GetComponent<WalletManager>().recepientAddressDropdown.value].text, wd.privateKey,
                GetComponent<WalletManager>().fundTransferAmountText.text));
        }

        else
        {
            GetComponent<WalletManager>().logText.Log("No Wallet Account Found");
        }
    }
    ...
    public T DecodeVariable<T>(string variableName, string result)
    {
        ...
        try
        {
            return function.DecodeTypeOutput<T>(result); // this results in an error if BigInteger is 0
        }
        catch
        {
            return default(T);
    ...
    public IEnumerator SendFunds(string addressFrom, string addressTo, string accountPrivateKey, string transferAmount)
    {
        ...
        var transactionSignedRequest = new TransactionSignedUnityRequest(GetComponent<WalletManager>().networkUrl,
            accountPrivateKey, BigInteger.Parse(addressFrom));

        // Then we send it and wait
        GetComponent<WalletManager>().logText.Log("Sending fund transfer transaction...");
        yield return transactionSignedRequest.SignAndSendTransaction(transactionInput);
        if (transactionSignedRequest.Exception == null)
        {
            // If we don't have exceptions we just display the result, congrats!
            GetComponent<WalletManager>().logText.Log("transfer tx submitted: " + transactionSignedRequest.Result);
            GetComponent<WalletManager>().CopyToClipboard(transactionSignedRequest.Result);
    ...
    public IEnumerator GetTokenInfo()
    {
        ...
        GetComponent<WalletManager>().AddInfoText("Token Address: \n" + TokenContractAddress, true);
        GetComponent<WalletManager>().AddInfoText("Name: " + TokenInfo.name + " (" + TokenInfo.symbol + ")");
        GetComponent<WalletManager>().AddInfoText("Decimals: " + TokenInfo.decimals);
        GetComponent<WalletManager>().AddInfoText("Total Supply: " + UnitConversion.Convert.FromWei(TokenInfo.totalSupply, TokenInfo.decimals) + " " + TokenInfo.symbol);

        GetComponent<WalletManager>().LoadingIndicator.SetActive(false);
    }
